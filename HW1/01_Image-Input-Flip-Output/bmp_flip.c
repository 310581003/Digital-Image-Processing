...
int main() {
    /*
        flip(path to input file, path to output file);
    */
    flip("../input2.bmp", "output2_flip.bmp");
 }

//--------------------------------------------------//
//                   Function                       //
//--------------------------------------------------//
int flip (const char *fname_s, const char *fname_t) {
    ...
    for(y = 0; y != height; ++y) {
        for(x = 0; x != width; ++x) {
                for (c=0; c!=n_channel; ++c){
                    ...
                    *(image_t + n_channel * (width * y + x_t) + c) = channel[c];
